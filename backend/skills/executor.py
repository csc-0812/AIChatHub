"""
技能执行器 - 基于 OpenClaw SDK

执行已加载的 OpenClaw Skill。
"""
import time
...
async def execute_skill(skill_name: str, **kwargs) -> SkillExecutionResult:
    ...
    try:
        ...
        if hasattr(skill, 'script') and skill.script:
            output = await _execute_script(skill, kwargs)
        elif hasattr(skill, 'user_prompt') and skill.user_prompt:
...
async def _validate_parameters(skill, parameters: Dict[str, Any]) -> None:
    """验证参数"""
...
async def _execute_script(skill, parameters: Dict[str, Any]) -> Any:
    """执行脚本"""
    try:
        exec_globals = {
        ...
        if 'result' in exec_globals and exec_globals['result'] != '':
...
async def _execute_prompt(skill, parameters: Dict[str, Any]) -> str:
    """执行提示词模板"""
    try:
